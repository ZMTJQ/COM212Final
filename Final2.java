	...
	public static void run2(){
		...
		try{
			...
			if(inputNum==1){
				...
				try{
				System.out.println("Enter ID to change the movie");
				int inputChangeM = in.nextInt();
					
				MovieNode m1 = movieIDDirectory.lookUp(inputChangeM);
				MovieNode m2 = movieDateDirectory.search(m1.getReleaseDate()); 
				System.out.println(m1.getName() + m1.getReleaseDate() + m1.getID());
				
				
				System.out.println("Enter new movie title:");
				String inputTitle = in.next();
				m1.setName(inputTitle);
				m2.setName(inputTitle);
				saveMovieBST(movieDateDirectory);
				saveMovieHash(movieIDDirectory);
				
				System.out.println("Enter new release date:");
				int inputRD = in.nextInt();
				m1.setReleaseDate(inputRD);
				m2.setReleaseDate(inputRD);
				saveMovieBST(movieDateDirectory);
				saveMovieHash(movieIDDirectory);
				
				System.out.println("Enter new rotton tomatos:");
				int inputRT = in.nextInt();
				m1.setRottenTomatoes(inputRT);
				m2.setRottenTomatoes(inputRT);
				saveMovieBST(movieDateDirectory);
				saveMovieHash(movieIDDirectory);
